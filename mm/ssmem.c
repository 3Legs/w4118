...
static inline void
ssmem_set_anon(struct ssmem_struct *ssmem, struct anon_vma *anon)
{
	ssmem->rmap = anon;
}
		    ...
		    struct ssmem_struct *data, void *addr)
{
	...
	pte_m =
	    get_locked_pte(vma_m->vm_mm,
			   (unsigned long) (vma_m->vm_start + addr -
					    vma_s->vm_start), &ptl_m);

	if (pte_none(*pte_m)) {
		__ssmem_fault_master(vma_m, data,
				     vma_m->vm_start + addr - vma_s->vm_start,
				     SLAVE_CALL);
	}
 ...
 */
static int
ssmem_fault(struct vm_area_struct *vma, struct vm_fault *vmf)
{
		  ...
		  struct vm_area_struct *target_vma)
{
	...
	for (offset = 0; offset < len; offset += PAGE_SIZE) {
		pte_source = get_locked_pte(source_vma->vm_mm,
					    source_start + offset, &ptl_source);
...
static void
__do_ssmem_munmap(struct vm_area_struct *vma)
{
...
static void
ssmem_close(struct vm_area_struct *area)
{
...
static inline struct ssmem_struct *
__create_ssmem(int id, size_t length)
{
...
SYSCALL_DEFINE3(ssmem_attach, int, id, int, flags, size_t, length)
{
	...
	if (atomic_read(&ssmem_count) >= SSMEM_MAX) {
		printk(KERN_ALERT
		       "ERROR in ssmem_attach: Too many ssmem exist.\n");
		return -EINVAL;
	}

	if (current->mm->map_count > sysctl_max_map_count) {
		printk(KERN_ALERT
		       "ERROR in ssmem_attach: Too many mappings!\n");
		return -ENOMEM;
	}
	...
	if (!SSMEM_TEST_ALLOC(id)) {
		if (!(flags & SSMEM_FLAG_CREATE)) {
			printk(KERN_ALERT
			       "ERROR in ssmem_attach: Invalid id!\n");
			return -EINVAL;
		}
		if (length == 0) {
			printk(KERN_ALERT
			       "ERROR in ssmem_attach: Invalid length!\n");
			return -EINVAL;
		}
	...
	for (;;) {
		vma =
		    find_vma_prepare(current->mm, addr, &prev, &rb_link,
				     &rb_parent);
