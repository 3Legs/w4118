...
static inline void __prepare_msghdr(struct msghdr *hdr, struct iovec *iov,
...
static enum clfs_status __read_response(struct socket *socket)
{
	...
	__prepare_msghdr(&hdr, &iov, &response, sizeof(enum clfs_status),
			 MSG_WAITALL);
	oldmm = get_fs();
	set_fs(KERNEL_DS);
	len = sock_recvmsg(socket, &hdr, sizeof(enum clfs_status), MSG_WAITALL);
	set_fs(oldmm);
...
static void __send_file_data_to_server(struct socket *socket,
				       struct inode *i_node)
{
...
static enum clfs_status __read_file_data_from_server(struct socket *socket,
						     struct inode *i_node)
{
	struct address_space *mapping = i_node->i_mapping;
	struct page *page;
