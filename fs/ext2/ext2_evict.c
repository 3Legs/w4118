 ...
 * __prepare_msghdr
 * 
 * prepare msghdr (iovec and hdr) for socket
 */
static inline void
__prepare_msghdr(struct msghdr *hdr, struct iovec *iov,
...
__prepare_addr(struct sockaddr_in *addr, struct inode *i)
{
	...
	addr->sin_port =
	    htons(((struct ext2_sb_info *) i->i_sb->s_fs_info)->port);
	addr->sin_addr.s_addr =
	    in_aton(((struct ext2_sb_info *) i->i_sb->s_fs_info)->ip);

	printk(KERN_ALERT "IP: %s, port: %d\n",
	       ((struct ext2_sb_info *) i->i_sb->s_fs_info)->ip,
	       ntohs(addr->sin_port));

}

static inline int
__connect_socket(struct socket *socket,
		 struct sockaddr_in *server_addr, struct inode *i)
{
	int r = 0;
...
static void
__send_request(struct socket *socket,
	       struct sockaddr_in *addr,
	       struct inode *i_node, enum clfs_type type)
{
	struct clfs_req *req;
	struct msghdr hdr;
	struct iovec iov;
	...
	__prepare_msghdr(&hdr, &iov, (void *) req,
			 sizeof (struct clfs_req), MSG_DONTWAIT);
...
static void
__send_response(struct socket *socket, enum clfs_status res)
{
	...
	__prepare_msghdr(&hdr, &iov, (void *) &response,
			 sizeof (enum clfs_status), MSG_DONTWAIT);
	oldmm = get_fs();
	set_fs(KERNEL_DS);
...
__read_response(struct socket *socket)
{
	...
	__prepare_msghdr(&hdr, &iov, &response,
			 sizeof (enum clfs_status), MSG_WAITALL);
	oldmm = get_fs();
	set_fs(KERNEL_DS);
	len =
	    sock_recvmsg(socket, &hdr, sizeof (enum clfs_status), MSG_WAITALL);
	set_fs(oldmm);

	if (len > 0)
		return response;
	printk(KERN_ALERT "Error in read response\n");
	return CLFS_ERROR;
}
...
__read_file_data_from_server(struct socket *socket, struct inode *i_node)
{
	...
	while (1) {
		...
		if (len < 0) {
			printk(KERN_ALERT "Receive error %d\n", i);
			goto read_out;
		}
...
int
ext2_evict(struct inode *i_node)
{
...
int
ext2_fetch(struct inode *i_node)
{
	struct sockaddr_in *server_addr = NULL;
	struct socket *socket;
	int r;
	...
	if (r == CLFS_OK) {
		printk(KERN_ALERT
		       "Got OK reponse, about to read file from server\n");
...
static int
time_greater(struct timespec *t1, struct timespec *t2)
{
...
static struct inode *
evict_ext2_iget(struct super_block *super, long ino)
{
...
ext2_evict_fs(struct super_block *super)
{
	...
	struct timespec *scan_time =
	    kmalloc(sizeof (struct timespec), GFP_KERNEL);
	struct timespec *set_time =
	    kmalloc(sizeof (struct timespec), GFP_KERNEL);
	struct timespec *current_time =
	    kmalloc(sizeof (struct timespec), GFP_KERNEL);
	struct clock_hand *clockhand =
	    kmalloc(sizeof (struct clock_hand), GFP_KERNEL);
	struct evicted *set_evicted =
	    kmalloc(sizeof (struct evicted), GFP_KERNEL);
	struct evicted *scan_evicted =
	    kmalloc(sizeof (struct evicted), GFP_KERNEL);

	int used_blocks =
	    ext2_es->s_blocks_count - ext2_count_free_blocks(super);
	...
	while (1) {
		...
		if (!S_ISREG(node->i_mode)
		    || atomic_read(&node->i_writecount) > 0)
			goto continue_loop_with_no_lock;
		...
		if (time_greater(scan_time, &node->i_atime)) {
			...
			res = ext2_xattr_get(node, EXT2_XATTR_INDEX_TRUSTED,
					     "evicted", scan_evicted,
					     sizeof (struct evicted));

			if (res >= 0 && scan_evicted->evicted == 1)
				goto continue_loop;

			printk(KERN_ALERT
			       "Calling ext2_evict, i_ino: %lu, i_writecount: %d free_blocks: %lu\n",
			       node->i_ino, atomic_read(&node->i_writecount),
			       ext2_count_free_blocks(super));
			...
			if (!res) {
				...
				res =
				    ext2_xattr_set(node,
						   ...
						   "evicted", set_evicted,
						   sizeof (struct evicted), 0);
				if (res < 0) {
					mutex_unlock(&node->i_mutex);
					printk(KERN_ALERT
					       "Error in ext2_xattr_set.\n");
					goto out;
				}
			}
			...
			if (utility < 10 * ext2_sup->evict) {
				...
				if (res < 0)
					printk(KERN_ALERT
					       "Error in ext2_xattr_set replace.\n");
